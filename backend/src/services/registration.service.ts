...
export class RegistrationService {
  ...
  async createRegistration(data: {
    ...
    if (period.status !== "OPEN")
      throw new AppError(
        "Đợt tuyển sinh không ở trạng thái nhận đơn",
        400,
        "PERIOD_NOT_OPEN",
      );
    ...
    if (existed)
      throw new AppError(
        "Học viên đã có đơn đăng ký cho đợt này",
        409,
  ...
  async reviewRegistration(
    ...
    if (reg.status !== "PENDING")
      throw new AppError(
        "Chỉ có thể xét duyệt phiếu đang ở trạng thái PENDING",
        400,
        "INVALID_STATUS",
      );
  ...
  async cancelRegistration(
    ...
    if (!isAdmin && reg.student.userId !== requestingUserId) {
      throw new AppError(
        "Không có quyền hủy đơn đăng ký này",
        403,
        "FORBIDDEN",
      );
    }
    if (reg.status === "CANCELLED")
      throw new AppError("Đơn đăng ký đã bị hủy rồi", 400, "ALREADY_CANCELLED");
    if (!isAdmin && reg.status !== "PENDING") {
      throw new AppError(
        "Chỉ có thể hủy đơn đang ở trạng thái chờ duyệt",
        400,
        "CANNOT_CANCEL",
      );
    }
